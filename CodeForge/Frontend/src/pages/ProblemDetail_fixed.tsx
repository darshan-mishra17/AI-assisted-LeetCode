...
public:
  ...
  const handleRunCode = async () => {
    ...
    try {
      ...
      const submission = await apiService.submitCode({
        problem_id: "test-problem-id", // Use a hardcoded ID for now
        code: code,
        language: language
      });
      ...
      const results: TestResult[] = [{
        ...
        input: `All test cases (${submission.total_test_cases})`,
        expected: `${submission.total_test_cases} test cases to pass`,
        actual: `${submission.test_cases_passed} test cases passed`,
        time: submission.runtime ? `${submission.runtime}ms` : '42ms',
  ...
  const handleSubmit = async () => {
    ...
    try {
      const submission = await apiService.submitCode({
        problem_id: "test-problem-id", // Use hardcoded ID for now
        code: code,
        language: language
      });
      ...
      if (submission.status === 'accepted') {
        ...
        // Trigger a refresh of dashboard data by dispatching a custom event
